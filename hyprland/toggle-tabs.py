...
if __name__ == "__main__":
    ...
    try:
        active_window = json.loads(
            subprocess.check_output("hyprctl -j activewindow", shell=True)
        )
        if len(active_window["grouped"]) > 0:
            ...
            window_on_active_space = [
                w for w in windows if w["workspace"]["id"] == active_space_id
            ]
            should_group_windows = [
                w for w in window_on_active_space if EXCLUDE_TITLE not in w["title"]
            ]
